...
The files "DPHiddenRegion.m" and "HBHiddenRegion.m" contain the result of the computation of the hidden region of the Double Pentagon and Hexagon Box topologies, respectively. 
...
In the t-channel basis we also check that the entire dependence on non-collinear partons (i.e. the complete dependence on $x$ and $y$) in the two-loop splitting amplitude $\mathbf{Sp}^{(2)}$ is captured by the hidden region. 
